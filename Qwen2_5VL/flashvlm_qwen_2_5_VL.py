...
class Qwen2_5_VLVisionSdpaAttention(nn.Module):
    ## Note: Current implementation is not support the SpdaAttention
    def __init__(self, dim: int, num_heads: int = 16) -> None:
        super().__init__()
        self.num_heads = num_heads
        self.qkv = nn.Linear(dim, dim * 3, bias=True)
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            if self.gradient_checkpointing and self.training:
                if layer_num == len_blocks-1:
                    hidden_states, logits, attn_key = self._gradient_checkpointing_func(
                        blk.__call__, hidden_states, cu_seqlens_now, None, position_embeddings, True
                    )
                else:
                    hidden_states, _, _  = self._gradient_checkpointing_func(
                        blk.__call__, hidden_states, cu_seqlens_now, None, position_embeddings, False
                    )
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                image_embeds, attn_logits, attn_key = self.visual(pixel_values, grid_thw=image_grid_thw)
        ...
        if position_ids is None and (attention_mask is None or attention_mask.ndim == 2):
            ...
            if select_pixel:
                ...
                if st_idx.numel() == 0:
                    ...
                    image_embeds = inputs_embeds[:, first:last+1].squeeze(0)  # (N0, D)
                    N0 = image_embeds.shape[0]
                    #用户指定保留token数量
                    N = 196
                    N = min(N, N0)
                    alpha = 0.5
                    ...
                    text_embeds = inputs_embeds[:, :first, :].squeeze(0)
                    # ---------- 2. Text–Image semantic similarity ----------
                    if text_embeds is not None and text_embeds.numel() > 0:
                        # image_embeds: (N, D)
                        # text_embeds:  (L_text, D)

                        img = F.normalize(image_embeds, dim=-1)
                        txt = F.normalize(text_embeds, dim=-1)

                        # ===== 🔸 Norm-based semantic gating（新增）=====
                        text_norm = text_embeds.norm(dim=-1)                 # (L_text,)
                        text_norm = text_norm / (text_norm.max() + 1e-6)     # normalize to [0,1]
                        txt = txt * text_norm.unsqueeze(-1)                  # re-weight tokens
                        # ================================================
                        # --- token-aware similarity matrix ---
                        sim_mat = img @ txt.T                         # (N, L_text)
                        # --- soft token selection (对齐你下面的逻辑) ---
                        weights = F.softmax(sim_mat / 0.05, dim=-1)   # token-wise
                        sim = (sim_mat * weights).sum(dim=-1)         # (N,)
                        ...
                        sim = torch.zeros_like(attn_norm.squeeze(0))
                    # ---------- 3. Log-domain fusion (完全一致) ----------
                    eps = 1e-6
                    ...
                    token_indices = score.argsort(descending=True)
                    important_indices = token_indices[:N_imp]
                    residual_indices = token_indices[N_imp:]
                    feat_norm = F.normalize(image_embeds, dim=-1)
                    while residual_indices.numel() > N_div:
                        R = residual_indices.shape[0]
                        r = min(8, R - N_div)
                        if r <= 0:
                            break
                        tokens = feat_norm[residual_indices]
                        a, b = tokens[::2], tokens[1::2]
                        if a.numel() == 0 or b.numel() == 0:
                            break
                        sim_rb = (a @ b.T).max(dim=-1).values
                        keep = sim_rb.argsort(descending=True)[r:]
                        residual_indices = torch.cat(
                            [
                                residual_indices[::2][keep],
                                residual_indices[1::2],
                            ],
                            dim=0,
                        )
                    keep_indices = torch.cat(
                        [important_indices, residual_indices],
                        dim=0
                    )
                    keep_indices = torch.sort(keep_indices).values  # (N,)
                    select_mask = torch.zeros(
                        N0, dtype=torch.bool, device=inputs_embeds.device
                    )
                    select_mask[keep_indices] = True
